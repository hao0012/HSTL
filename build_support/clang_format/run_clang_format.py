...
def check(arguments, source_dir):
    ...
    for directory, subdirs, filenames in os.walk(source_dir):
        ...
        source_files = [x for x in fullpaths if x.endswith(".h") or x.endswith(".cpp") or x.endswith(".hpp")]
    ...
    # if len == 0, will run `clang-format -i`，cause: error: -i cannot read from stdin
